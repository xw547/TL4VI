...
for rho in rho_values:
    ...
    file_name = f"extra_large_sincos_nosq_xgb_simulation_wvar_results_1e_3_rho_{rho}.csv"
    results_df.to_csv(file_name, index=False)
    print(f"Simulation complete for rho={rho}. Results saved to {file_name}.")

    # Log failures
    if failed_results:
        failed_file_name = f"extra_large_sincos_nosq_xgb_wvar_failures_rho_{rho}.log"
