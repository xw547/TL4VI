...
def simulate_with_seed_and_rho(seed, rho, n=1000, N=500, p=10):
    try:
        ...
        full_model = MLPRegressor(
            hidden_layer_sizes=(32, 16),  # two hidden layers with 100 and 50 neurons respectively
            activation='relu',
            solver='adam',
            max_iter=3000,
            random_state=seed
        ...
        reduced_x = LinearRegression() 
        reduced_x.fit(full_data.iloc[:, 2:], full_data["X1"])   
        ...
        plug_in_var = np.mean((eif_now.iloc[:, 0] - eif_now.iloc[:, 1] + eif_now.iloc[:, 2])**2)
        ...
        tmle_var = np.mean((eif_now.iloc[:, 0] - eif_now.iloc[:, 1] + eif_now.iloc[:, 2])**2)
...
for rho in rho_values:
    ...
    file_name = f"single_zhou_mlp_simulation_wvar_results_1e_3_rho_{rho}.csv"
    results_df.to_csv(file_name, index=False)
    print(f"Simulation complete for rho={rho}. Results saved to {file_name}.")

    # Log failures
    if failed_results:
        failed_file_name = f"single_zhou_mlp_simulation_wvar_failures_rho_{rho}.log"
