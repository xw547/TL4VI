...
def simulate_with_seed_and_rho(seed, rho, n=1000, N=500, p=10):
    try:
        ...
        # Empirical densities
        reduced_den_y = empirical_rf_pdf(forest=reduced_model, X_train=full_data.iloc[:, 2:], X_test=full_data.iloc[:, 2:], seed=seed+2024)
...
for rho in rho_values:
    ...
    file_name = f"simple_simple_simulation_wvar_results_1e_3_rho_{rho}.csv"
    results_df.to_csv(file_name, index=False)
    print(f"Simulation complete for rho={rho}. Results saved to {file_name}.")

    # Log failures
    if failed_results:
        failed_file_name = f"simple_simple_simulation_wvar_failures_rho_{rho}.log"
