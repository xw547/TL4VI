...
def simulate_with_seed_and_rho(seed, rho, n=1000, N=500, p=10):
    try:
        ...
        full_model = MLPRegressor(
            hidden_layer_sizes=(64, 32, 16, 8),  # two hidden layers with 100 and 50 neurons respectively
            activation='relu',
            solver='adam',
            max_iter=3000,
            random_state=seed
...
for rho in rho_values:
    ...
    file_name = f"extra_large_sincos_nosq_mlp_simulation_wvar_results_1e_3_rho_{rho}.csv"
    results_df.to_csv(file_name, index=False)
    print(f"Simulation complete for rho={rho}. Results saved to {file_name}.")

    # Log failures
    if failed_results:
        failed_file_name = f"extra_large_sincos_nosq_mlp_wvar_failures_rho_{rho}.log"
