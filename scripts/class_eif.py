...
def logit_class_eif_cpi_empirical(full_data, reduced_den_y, reduced_den_x, N, full_model, seed):
    """
    ...
    # Assuming full_model supports `.predict`
    full_pred = full_model.predict(full_data.iloc[:, 1:])
    Y = full_data.iloc[:, 0]
    np.random.seed(seed)
    for i in range(n):
        # Bootstrapped samples for Y and X
        curr_y = np.random.choice(
            full_data.iloc[:, 0], size=N, replace=True, p=reduced_den_y[i])
        curr_x = np.random.choice(
            full_data.iloc[:, 1], size=N, replace=True, p=reduced_den_x[i])

        # Create new data for predictions
        second_data = pd.DataFrame(
            np.tile(full_data.iloc[i].values, (N, 1)), columns=full_data.columns)
        ...
        first_term[i] = np.mean(-Y[i] * np.log(full_pred[i]) -
                                (1 - Y[i]) * np.log(1 - full_pred[i]))
        second_term[i] = np.mean(-curr_y * np.log(second_pred) -
                                 (1 - curr_y) * np.log(1 - second_pred))
        third_term[i] = np.mean(-Y[i] * np.log(second_pred) -
                                (1 - Y[i]) * np.log(1 - second_pred))
...
def true_class_eif_cpi_empirical(full_data, reduced_den_y, reduced_den_x, N, full_model, seed):
    """
    Compute different terms of Efficient Influence Function (EIF) for Conditional Predictive Importance (CPI).
